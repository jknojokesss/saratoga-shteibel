...
import { paymentInstructionBlocks } from './ykPaymentInfo'
...
export function buildSaleInvoicePdf({ data, buyer }) {
  ...
  if (balance <= 0) {
    ...
    paymentInstructionBlocks.forEach(({ label, value }, i) => {
      if (i > 0) y += 6
      doc.font('SansMedium').fontSize(8).fillColor(NAVY).text(label, M, y)
      y += 12
      doc.font('Sans').fontSize(9).fillColor(MUTED).text(value, M, y, { width: W, lineGap: 2 })
      y += 14
    })
  }
